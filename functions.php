...
$gf_id = '1'; // your GF ID
add_action("gform_after_submission_$gf_id", "gravity_post_submission", 10, 2);
function gravity_post_submission ($entry, $form){

	$post_id = $entry["post_id"]; // example purposes, may want to sanitise first!
  $strCategories = "";

  //echo "Post ID: ".$post_id;

  //field_name_in_gf
	$values = get_post_custom_values("category", $post_id);
	foreach ( $values as $key => $value ) {
		$strCategories .= $value . ", ";
	}
  //echo "Categories: ".$strCategories;

  //acf_field_key
	update_field("category", $values, $post_id);

}

$gf_id = '3'; // your GF ID
add_action("gform_after_submission_$gf_id", "gravity_post_submission_2", 10, 2);
function gravity_post_submission_2 ($entry, $form){
